...
def test_apply_unified_ai_result_creates_beats(project):
    ...
    service.import_result(
        project, step="generate-unified-package", result_data=result_data, episode_id="ep_001"
    )
...
def test_apply_unified_ai_result_rejects_invalid_json(project):
    ...
    msg = service.import_result(
        project, step="generate-unified-package", result_data={}, episode_id="ep_001"
    )
...
def test_beat_studio_no_primary_offline_generate_package_button():
    ...
    # QApplication must exist for any Qt widget to be constructed; we don't
    # use the reference after creation.
