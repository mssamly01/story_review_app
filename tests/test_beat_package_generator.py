...
def project():
    ...
    # Add Style
    style = StylePreset(
        style_id="test_style", name="Test Style", positive_prompt="high quality illustration"
    )
...
def test_beat_package_generator_deterministic_scene(project):
    ...
    assert (
        project.source_chapters[0].raw_text
        == "The hero entered the castle. He looked around in awe."
    )
...
def test_beat_package_generator_mock_ai_scene(project):
    ...
    beats = service.generate_unified_package_for_scene(project, "ep_001", "sc_001", use_ai=True)

    assert len(beats) == 2
    assert (
        beats[0].review_text
        == "Ngay lúc này, nhân vật chính chậm lại và nhận ra một chi tiết quan trọng."
    )
...
def test_beat_package_generator_episode(project):
    ...
    service = BeatGeneratorService()
    beats = service.generate_unified_package_for_episode(project, "ep_001", use_ai=False)
