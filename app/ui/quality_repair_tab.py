...
class QualityRepairTab(QWidget):
    ...
    def _on_readiness(self) -> None:
        ...
        try:
            ...
            self.results_table.setItem(
                0, 1, QTableWidgetItem("Sẵn sàng" if report.get("is_ready") else "Chưa sẵn sàng")
            )
    ...
    def _on_repair(self) -> None:
        ...
        try:
            ...
            self.results_table.setItem(
                0, 2, QTableWidgetItem(f"Đã áp dụng {len(applied)} sửa lỗi rủi ro thấp.")
            )
    ...
    def _show_issues(self, issues) -> None:
        ...
        for row, issue in enumerate(issues):
            self.results_table.setItem(
                row, 0, QTableWidgetItem(getattr(issue, "category", "unknown"))
            )
            self.results_table.setItem(row, 1, QTableWidgetItem(getattr(issue, "severity", "info")))
            self.results_table.setItem(
                row, 2, QTableWidgetItem(getattr(issue, "message", str(issue)))
            )
    ...
    def _show_suggestions(self, suggestions) -> None:
        ...
        for row, s in enumerate(suggestions):
            ...
            self.results_table.setItem(
                row, 1, QTableWidgetItem(getattr(s, "risk_level", "unknown"))
            )
